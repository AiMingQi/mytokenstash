    ...
    <v-container>
        <v-row>
            ...
            <v-col cols="12" sm="6">
                <v-card v-show="!validSolanaAddress">
                    <v-card-text>
                        <h2 >Input Valid NFT Token Mint Address</h2>
                    </v-card-text>
                    <v-card-text>
                        <v-col cols="12">
                        <v-text-field 
                            v-model="nftMintAccount" 
                            :rules="[rules.required, rules.counter]"
                            width="100%"
                            auto-grow
                            rows="2"
                ...
                <v-card v-show="validSolanaAddress">
                    ...
                    <v-card-text>
                        ...
                        {{solanaLookupAccount}} {{nftMintAccount}} 
                    </v-card-text>
                    <v-card-text>
                        <h2 class="green--text float-left" ><v-icon>mdi-check</v-icon> Is Unique </h2>
                        Amount In Circulation = {{nftAccountInfo.value.amount}}
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn class="white--text ml-2" color="purple" @click="reset"  >Reset</v-btn>
                    </v-card-actions>
                </v-card>
            </v-col>
        </v-row>
...
<script>
  ...
  import { clusterApiUrl, solanaWeb3, PublicKey } from "@solana/web3.js";
    ...
    methods: {
        ...
        async checkSolanaAddress () {
            try {
                ...
                if (result === true) {
                    this.getAllNftData(this.solanaLookupAccount);
        ...
        async checkNftTokenAddress () {
            try {
                const connection = createConnectionConfig(clusterApiUrl(this.$store.state.network));
                const result = isValidSolanaAddress(this.nftMintAccount);
                console.log("result", result);
                if (result === true) {
                    // this.getAllNftData(this.nftMintAccount);
                    let nftAccountInfo = await connection.getTokenSupply(new PublicKey(this.nftMintAccount));
                    ...
                    // console.log(block);
                    this.nftAccountInfo = nftAccountInfo;
                    this.validSolanaAddress = true;
